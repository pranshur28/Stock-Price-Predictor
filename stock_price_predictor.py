...
This script builds machine learning models to predict stock/ETF prices using historical data.
It implements multiple regression approaches for prediction and works with any valid ticker symbol.
"""
...
class StockPricePredictor:
    def __init__(self, ticker, start_date=None, end_date=None, prediction_days=30):
        """
        ...
        -----------
        ticker : str
            Stock ticker symbol (e.g., 'AAPL', 'MSFT', 'QQQ')
        ...
        os.makedirs(f'results/{self.ticker}', exist_ok=True)
    ...
    def fetch_data(self):
        ...
        if self.data.empty or len(self.data) < 100:
            raise ValueError(f"Not enough data fetched for {self.ticker}. Check your ticker symbol, date range, and internet connection.")
        ...
        print(f"Successfully fetched {len(self.data)} days of {self.ticker} data")
    ...
    def train_regression_models(self):
        ...
        for name, model in models.items():
            ...
            # Save the model
            with open(f'models/{self.ticker}_{name}_model.pkl', 'wb') as f:
                pickle.dump(model, f)
    ...
    def _plot_feature_importance(self):
        ...
        plt.title(f'Feature Importance for {self.ticker} (Random Forest)')
        plt.tight_layout()
        plt.savefig(f'results/{self.ticker}/feature_importance.png')
        plt.close()
    ...
    def _compare_models(self, results, y_test):
        ...
        # Save the comparison
        comparison_df.to_csv(f'results/{self.ticker}/model_comparison.csv', index=False)
        ...
        plt.tight_layout()
        plt.savefig(f'results/{self.ticker}/model_comparison.png')
        plt.close()
        ...
        plt.title(f'{self.ticker} Price Prediction: Model Comparison')
        plt.xlabel('Date')
        plt.ylabel('Price ($)')
        plt.legend()
        plt.grid(True)
        plt.xticks(rotation=45)
        plt.tight_layout()
        plt.savefig(f'results/{self.ticker}/all_models_prediction.png')
        plt.close()
    ...
    def plot_predictions(self, actual, predictions, title='Price Predictions'):
        ...
        plt.legend()
        plt.grid(True)
        plt.savefig(f'results/{self.ticker}/{title.lower().replace(" ", "_")}.png')
        plt.close()
...
def main():
    ...
    parser.add_argument('ticker', type=str, help='Stock ticker symbol (e.g., AAPL, MSFT, QQQ)')
    ...
    parser.add_argument('--prediction-days', type=int, default=7, help='Number of days to predict')
    parser.add_argument('--model', type=str, default='best', help='Model to use for prediction (linear_regression, ridge, lasso, elastic_net, svr, random_forest, or best)')
    
    args = parser.parse_args()
    
    print(f"{args.ticker} Price Predictor")
    print("=" * (len(args.ticker) + 16))
    ...
    if args.model == 'best':
        ...
        if args.model in results:
            ...
            print(f"Specified model '{args.model}' not found. Using best model instead.")
            best_model = max(results.items(), key=lambda x: x[1]['r2'])[0]
            model_to_use = best_model
            print(f"Best model based on R² score: {model_to_use.replace('_', ' ').title()}")
    
    # Make predictions for the next days
    next_days = args.prediction_days
    predictions = predictor.predict_next_days(days=next_days, model_name=model_to_use)
    
    # Get the last 30 actual prices for plotting
    last_prices = predictor.processed_data['Close'].values[-30:]
    
    # Plot predictions
    predictor.plot_predictions(last_prices, predictions, f'{model_to_use.replace("_", " ").title()} Predictions')
    
    # Print predictions
    print(f"\nPredictions for the next {next_days} days using {model_to_use.replace('_', ' ').title()}:")
    for i, pred in enumerate(predictions):
        print(f"Day {i+1}: ${pred:.2f}")
    
    print(f"\nAnalysis complete! Check the 'results/{args.ticker}' folder for visualizations.")
